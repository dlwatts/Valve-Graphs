...
valveRecent <- function(userInput, readings = 3 * 24 * 6, offset = 0, limit = 1000) {
	#required libraries
	library(jsonlite)
	library(xts)
...
ownerInfoLink <- paste("https://edynpepper.firebaseio.com/users_gardens/", ownerID, ".json?auth=Uj9kHlU25azPe1jH9Xq75I9yDv78pKku4tHOdTEs", 
	...
	for (i in 1:length(gardenIDs)) {
		...
		if (any(device == "valve")) {
			...
			png(paste(gardenDirectory, "/", "valveRecent", "_", current.time, ".png", sep = ""), width = 800, height = 480)
			...
			if (!is.na(sensorID)) {
				...
				t1 = min(data[["timestamp"]])
				t2 = max(data[["timestamp"]])
				ts.axis <- as.POSIXct(seq(t1, t2, (12 * 60 * 60)), origin = "1970-01-01", tz = TZ)
				...
				axis.POSIXct(1, at = ts.axis, labels = format(ts.axis, "%m/%d\n %H:%M"), las = 0)
				text(t1+5, 8, paste("last sensor reading @", as.POSIXct(t2, origin = "1970-01-01", tz = TZ)), pos = 4)
			...
			if (is.na(sensorID)) {
				...
				t1 <- min(valveHistJSON$timestamp)
				t2 <- max(valveHistJSON$timestamp)
				ts.axis <- as.POSIXct(seq(t1, t2, (12 * 60 * 60)), origin = "1970-01-01", tz = TZ)
			...
			text(t1+5, 10, paste("last valve reading @", as.POSIXct(max(valveHistJSON[["timestamp"]]), origin = "1970-01-01", tz = TZ)), pos = 4)


			# -------Legend-------			
			legend("topright", c("soil moisture", "past irrigation (min)", "future irrigation (min)"), pch = c(20, 
				35, 35), lty = c(0, 1, 1), col = c("skyblue", "royalblue4", "darkred"), box.lwd = 0, box.col = "white", bg = "white")
			dev.off()
		}
	}
}
