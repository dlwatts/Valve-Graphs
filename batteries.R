...
batteries <- function(userInput, readings = 14 * 24 * 6, offset = 0, limit = 2000) {
	#required libraries
	library(jsonlite)
	library(xts)
...
ownerInfoLink <- paste("https://edynpepper.firebaseio.com/users_gardens/", ownerID, ".json?auth=Uj9kHlU25azPe1jH9Xq75I9yDv78pKku4tHOdTEs", 
	...
	for (i in 1:length(gardenIDs)) {
		...
		if (any(device == "valve")) {
			...
			#smart title
			tit <- paste("valve:", valveID, "sensor:", sensorID, "batteries")

			# -------Now we're ready to pull and plot the sensor data-------			
			#set up the plot output
			png(paste(gardenDirectory, "/", "batteries", "_", current.time, ".png", sep = ""), width = 800, height = 480)
			...
			if (!is.na(sensorID)) {
				...
				plot(data[["battery"]] ~ data[["timestamp"]], main = tit, ylab = yaxis, xlim = c(t1, t2), xaxt = "n", 
					pch = 20, type = "p", xlab = "", ylim = c(0, 3.85), col = rgb(205/255, 38/255, 38/255, 0.5))
				axis.POSIXct(1, at = ts.axis, labels = format(ts.axis, "%m/%d"), las = 2)
			} else {
				#empty plot if there is no linked sensor
				plot(0, main = tit, ylim = c(0, 3.8), xlim = c(t1, t2), xaxt = "n", xlab = "", ylab = yaxis)
				axis.POSIXct(1, at = ts.axis, labels = format(ts.axis, "%m/%d"), las = 2)
			}
			...
			Valve$time <- as.POSIXct(Valve$timestamp, origin = "1970-01-01", tz = TZ)
			points(Valve[["battery"]] ~ Valve[["time"]], pch = 20, col = rgb(139/255, 26/255, 26/255, 0.5))

			# -------Legend-------			
			legend("bottomright", c("sensor battery", "valve battery"), pch = c(20, 20), col = c("firebrick3", "firebrick4"), 
