...
valveMonth <- function(userInput, readings = 30 * 24 * 6, offset = 0, limit = 1000) {
	#required libraries
	library(jsonlite)
	library(xts)
...
ownerInfoLink <- paste("https://edynpepper.firebaseio.com/users_gardens/", ownerID, ".json?auth=Uj9kHlU25azPe1jH9Xq75I9yDv78pKku4tHOdTEs", 
	...
	for (i in 1:length(gardenIDs)) {
		...
		if (any(device == "valve")) {
			...
			png(paste(gardenDirectory, "/", "valveMonth", "_", current.time, ".png", sep = ""), width = 800, height = 480)
			...
			# -------Legend-------			
			legend("topright", c("soil moisture", "past irrigation (min)"), pch = c(20, 35), lty = c(0, 1), col = c("skyblue", 
				"royalblue4"), box.lwd = 0, box.col = "white", bg = "white")
			dev.off()
		}
	}
}